...
Call log:
  - waiting for locator('#pact1').contentFrame().locator('#inp_val').first()
...
```ts
  ...
  14 | 
> 15 | await Frame1.locator('#inp_val').first().fill('SRK');
     |                                          ^ Error: locator.fill: Error: strict mode violation: locator('#pact1') resolved to 2 elements:
  16 | 
  17 | await Frame2.locator('#jex').fill('ARI');
  18 | await Frame2.locator('#glaf').fill('Heaven');
  19 | 
  20 | 
  21 |  const headerText = await  Frame1.locator('h3').innerText();
  22 | 
  23 |  console.log(headerText);
  24 | 
  25 | });
  26 | 
  27 | 
