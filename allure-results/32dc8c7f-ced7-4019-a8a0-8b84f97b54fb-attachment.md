# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: 09_Frmae_iframe\241_Iframe_Within_Iframe.spec.ts >> Basic multi frame  Test - Interact with multiple frames
- Location: tests\09_Frmae_iframe\241_Iframe_Within_Iframe.spec.ts:3:5

# Error details

```
Error: locator.fill: Error: strict mode violation: locator('#pact1') resolved to 2 elements:
    1) <iframe id="pact1" height="400px" src="https://selectorshub.com/iframe-and-nested-iframe/"></iframe> aka locator('#pact1').first()
    2) <iframe id="pact1" height="400px" src="https://selectorshub.com/iframe-and-nested-iframe/"></iframe> aka locator('section #pact1')

Call log:
  - waiting for locator('#pact1').contentFrame().getByTitle('Enter your first crush name')

```

# Page snapshot

```yaml
- generic [active] [ref=e1]:
  - link "Skip to content" [ref=e2] [cursor=pointer]:
    - /url: "#content"
  - banner [ref=e3]:
    - banner [ref=e4]:
      - generic [ref=e5]:
        - heading "Get free access to our advanced XPath & CSS course. Ends in -" [level=5] [ref=e7]:
          - link "Get free access to our advanced XPath & CSS course. Ends in -" [ref=e8] [cursor=pointer]:
            - /url: https://selectorshub.com/get-free-access-of-advanced-xpath-and-css-selector-paid-course/
        - generic [ref=e10]:
          - generic [ref=e11]: 10 Hrs
          - generic [ref=e12]: 00 Min
          - generic [ref=e13]: 00 Sec
        - link "Claim It Here" [ref=e15] [cursor=pointer]:
          - /url: https://selectorshub.com/get-free-access-of-advanced-xpath-and-css-selector-paid-course/
          - generic [ref=e17]: Claim It Here
      - generic [ref=e20]:
        - link [ref=e22] [cursor=pointer]:
          - /url: https://selectorshub.com
        - navigation "Menu" [ref=e24]:
          - generic "Menu Toggle" [ref=e25]:
            - list [ref=e26]:
              - listitem [ref=e27]:
                - generic [ref=e28]:
                  - generic [ref=e30]: Products
                  - button "Open Products" [ref=e31]:
                    - generic [ref=e32]:
                      - img [ref=e33]
                      - generic [ref=e35]: Open Products
              - listitem [ref=e36]:
                - generic [ref=e37]:
                  - generic [ref=e39]: Pro Plans
                  - button "Open Pro Plans" [ref=e40]:
                    - generic [ref=e41]:
                      - img [ref=e42]
                      - generic [ref=e44]: Open Pro Plans
              - listitem [ref=e45]:
                - link "Courses" [ref=e47] [cursor=pointer]:
                  - /url: /courses-recordings/
                  - generic [ref=e48]: Courses
              - listitem [ref=e49]:
                - link "Practice Page" [ref=e51] [cursor=pointer]:
                  - /url: /xpath-practice-page/
                  - generic [ref=e52]: Practice Page
              - listitem [ref=e53]:
                - generic [ref=e54]:
                  - generic [ref=e56]: Resources
                  - button "Open Resources" [ref=e57]:
                    - generic [ref=e58]:
                      - img [ref=e59]
                      - generic [ref=e61]: Open Resources
              - listitem [ref=e62]:
                - link "Help?" [ref=e64] [cursor=pointer]:
                  - /url: /contact-us/
                  - generic [ref=e65]: Help?
        - link [ref=e68] [cursor=pointer]:
          - /url: "#elementor-action%3Aaction%3Dpopup%3Aopen%26settings%3DeyJpZCI6IjIyNjU3IiwidG9nZ2xlIjpmYWxzZX0%3D"
          - img [ref=e69]
  - main [ref=e71]:
    - generic [ref=e76]:
      - heading "iframe and nested iframes" [level=6] [ref=e78]
      - iframe [ref=e81]:
        - generic [active] [ref=f1e1]:
          - generic [ref=f1e7]:
            - textbox "Enter your first crush name" [ref=f1e8]:
              - /placeholder: First Crush
            - button "Lost" [ref=f1e9]
            - link "Get Free Access to Advanced XPath & CSS Selector Course" [ref=f1e10] [cursor=pointer]:
              - /url: https://selectorshub.com/get-free-access-of-advanced-xpath-and-css-selector-paid-course/
            - heading "Dare for you" [level=3] [ref=f1e11]
            - iframe [ref=f1e12]:
              - generic [active] [ref=f3e1]:
                - generic [ref=f3e7]:
                  - textbox "Current Crush Name" [ref=f3e8]
                  - button "Connect now" [ref=f3e9]
                  - heading "Heaven is here" [level=4] [ref=f3e10]
                  - iframe [ref=f3e11]:
                    - generic [active] [ref=f5e1]:
                      - generic [ref=f5e7]:
                        - textbox "Destiny" [ref=f5e8]
                        - button "Close it" [ref=f5e9]
                      - generic [ref=f5e10]: mobile
                - generic [ref=f3e12]: mobile
          - generic [ref=f1e13]: tablet
  - contentinfo [ref=e82]:
    - contentinfo [ref=e83]:
      - generic [ref=e84]:
        - generic [ref=e85]:
          - generic [ref=e86]:
            - link "SelectorHub Logo" [ref=e88] [cursor=pointer]:
              - /url: https://selectorshub.com
              - img "SelectorHub Logo" [ref=e89]
            - generic [ref=e90]:
              - paragraph [ref=e91]: We’re Here to Enhance Your Web Testing—Start Now
              - paragraph [ref=e92]: support@selectorshub.com
          - generic [ref=e93]:
            - heading "Free Tools" [level=4] [ref=e95]
            - navigation "Menu" [ref=e97]:
              - list [ref=e98]:
                - listitem [ref=e99]:
                  - link "SelectorsHub – Best Tool to auto generate Playwright Locators and Xpath" [ref=e100] [cursor=pointer]:
                    - /url: https://selectorshub.com/selectorshub/
                - listitem [ref=e101]:
                  - link "TestCase Studio" [ref=e102] [cursor=pointer]:
                    - /url: https://selectorshub.com/testcase-studio/
                - listitem [ref=e103]:
                  - link "Check My Links" [ref=e104] [cursor=pointer]:
                    - /url: https://selectorshub.com/checkmylinks/
                - listitem [ref=e105]:
                  - link "Exploratory Tester" [ref=e106] [cursor=pointer]:
                    - /url: https://selectorshub.com/exploratory-tester/
                - listitem [ref=e107]:
                  - link "Screenshot with URL" [ref=e108] [cursor=pointer]:
                    - /url: https://selectorshub.com/screenshot-with-url/
                - listitem [ref=e109]:
                  - link "Page Load Timer" [ref=e110] [cursor=pointer]:
                    - /url: https://selectorshub.com/page-load-timer/
                - listitem [ref=e111]:
                  - link "Testing Daily" [ref=e112] [cursor=pointer]:
                    - /url: https://selectorshub.com/testing-daily/
                - listitem [ref=e113]:
                  - link "Auto Data Filler" [ref=e114] [cursor=pointer]:
                    - /url: https://selectorshub.com/auto-data-filler/
                - listitem [ref=e115]:
                  - link "AutoTestData" [ref=e116] [cursor=pointer]:
                    - /url: https://selectorshub.com/autotestdata/
                - listitem [ref=e117]:
                  - link "Automation Tool Analyzer" [ref=e118] [cursor=pointer]:
                    - /url: https://selectorshub.com/automation-tool-analyzer/
                - listitem [ref=e119]:
                  - link "AI Post Creator" [ref=e120] [cursor=pointer]:
                    - /url: https://selectorshub.com/ai-post-creator/
          - generic [ref=e121]:
            - heading "Paid Tools" [level=4] [ref=e123]
            - navigation "Menu" [ref=e125]:
              - list [ref=e126]:
                - listitem [ref=e127]:
                  - link "SelectorsHub Pro" [ref=e128] [cursor=pointer]:
                    - /url: https://selectorshub.com/selectorshub-pro/
                - listitem [ref=e129]:
                  - link "TestCase Studio Pro" [ref=e130] [cursor=pointer]:
                    - /url: https://selectorshub.com/testcase-studio-pro/
                - listitem [ref=e131]:
                  - link "Promote with us" [ref=e132] [cursor=pointer]:
                    - /url: https://selectorshub.com/selectorshub-ads/
          - generic [ref=e133]:
            - heading "Resources" [level=4] [ref=e135]
            - navigation "Menu" [ref=e137]:
              - list [ref=e138]:
                - listitem [ref=e139]:
                  - link "Courses" [ref=e140] [cursor=pointer]:
                    - /url: https://selectorshub.com/courses-recordings/
                - listitem [ref=e141]:
                  - link "Trainings" [ref=e142] [cursor=pointer]:
                    - /url: https://selectorshub.com/bootcamp/
                - listitem [ref=e143]:
                  - link "Video Tutorials" [ref=e144] [cursor=pointer]:
                    - /url: https://www.youtube.com/c/SelectorsHub?sub_confirmation=1
                - listitem [ref=e145]:
                  - link "Meetup" [ref=e146] [cursor=pointer]:
                    - /url: https://selectorshub.com/meetup/
                - listitem [ref=e147]:
                  - link "SelectorsHub Offers" [ref=e148] [cursor=pointer]:
                    - /url: https://selectorshub.com/offers/
          - generic [ref=e149]:
            - heading "Useful Links" [level=4] [ref=e151]
            - navigation "Menu" [ref=e153]:
              - list [ref=e154]:
                - listitem [ref=e155]:
                  - link "About Us" [ref=e156] [cursor=pointer]:
                    - /url: https://selectorshub.com/team/
                - listitem [ref=e157]:
                  - link "Contact Us" [ref=e158] [cursor=pointer]:
                    - /url: https://selectorshub.com/contact-us/
                - listitem [ref=e159]:
                  - link "Sponsors" [ref=e160] [cursor=pointer]:
                    - /url: https://selectorshub.com/sponsors/
                - listitem [ref=e161]:
                  - link "Practice Page" [ref=e162] [cursor=pointer]:
                    - /url: https://selectorshub.com/xpath-practice-page/
                - listitem [ref=e163]:
                  - link "Cancellation & Refund Policy" [ref=e164] [cursor=pointer]:
                    - /url: https://selectorshub.com/cancellation-refund-policy/
                - listitem [ref=e165]:
                  - link "Certification" [ref=e166] [cursor=pointer]:
                    - /url: https://selectorshub.com/certification/
                - listitem [ref=e167]:
                  - link "Privacy Policy" [ref=e168] [cursor=pointer]:
                    - /url: https://selectorshub.com/privacy-policy/
                - listitem [ref=e169]:
                  - link "Terms of Service" [ref=e170] [cursor=pointer]:
                    - /url: https://selectorshub.com/terms-of-service/
        - generic [ref=e171]:
          - generic [ref=e173]: Copyright © 2026 SelectorsHub ® . All rights reserved
          - list [ref=e176]:
            - listitem [ref=e177]:
              - link "Youtube" [ref=e178] [cursor=pointer]:
                - /url: https://www.youtube.com/c/SelectorsHub?sub_confirmation=1
                - generic [ref=e179]: Youtube
                - img [ref=e180]
            - listitem [ref=e182]:
              - link "Telegram" [ref=e183] [cursor=pointer]:
                - /url: https://t.me/selectorshub
                - generic [ref=e184]: Telegram
                - img [ref=e185]
            - listitem [ref=e187]:
              - link "Instagram" [ref=e188] [cursor=pointer]:
                - /url: https://www.instagram.com/SelectorsHub/
                - generic [ref=e189]: Instagram
                - img [ref=e190]
            - listitem [ref=e192]:
              - link "Linkedin" [ref=e193] [cursor=pointer]:
                - /url: https://www.linkedin.com/company/selectorshub/about
                - generic [ref=e194]: Linkedin
                - img [ref=e195]
            - listitem [ref=e197]:
              - link "Facebook" [ref=e198] [cursor=pointer]:
                - /url: https://www.facebook.com/selectorshub/
                - generic [ref=e199]: Facebook
                - img [ref=e200]
            - listitem [ref=e202]:
              - link "Twitter" [ref=e203] [cursor=pointer]:
                - /url: https://x.com/SelectorsHub
                - generic [ref=e204]: Twitter
                - img [ref=e205]
  - generic [ref=e207]: desktop
```

# Test source

```ts
  1  | import { test, expect, FrameLocator, Locator } from '@playwright/test';
  2  | 
  3  | test('Basic multi frame  Test - Interact with multiple frames', async ({ page }) => {
  4  | 
  5  | await page.goto('https://selectorshub.com/iframe-scenario/');
  6  | 
  7  | let Frame1 :FrameLocator =  page.frameLocator('#pact1');
  8  | 
  9  | let Frame2 :FrameLocator =  Frame1.frameLocator('#pact2');
  10 | 
  11 | let Frame3 :FrameLocator =  Frame2.frameLocator('#pact3');
  12 | 
> 13 | await Frame1.getByTitle('Enter your first crush name').fill('SRK');
     |                                                        ^ Error: locator.fill: Error: strict mode violation: locator('#pact1') resolved to 2 elements:
  14 | 
  15 | await Frame2.locator('#jex').fill('ARI');
  16 | await Frame2.locator('#glaf').fill('Heaven');
  17 | 
  18 | 
  19 |  const headerText = await  Frame1.locator('h3').innerText();
  20 | 
  21 |  console.log(headerText);
  22 | 
  23 | });
  24 | 
  25 | 
  26 | 
  27 | 
  28 | 
```