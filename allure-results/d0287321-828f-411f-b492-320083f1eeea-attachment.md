...
Call log:
  - waiting for locator('#frame-one').contentFrame().getByPlaceholder('MH-12-AB-12342')
...
```ts
  ...
  6  | 
  7  |    await page.frameLocator('#frame-one').locator('#RESULT_TextField-1').fill("Creta");
  8  | 
  9  |     await page.frameLocator('#frame-one').locator('#RESULT_TextField-2').fill("Harshita");
  10 |    
> 11 |     await page.frameLocator('#frame-one').getByPlaceholder('MH-12-AB-12342').fill("MH-12-AB-2612");
     |                                                                              ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  12 | 
  13 | 
